...
// Add click event listeners for language filters
document.querySelectorAll('.language-list .genre-item').forEach(item => {
    item.addEventListener('click', () => {
        // Remove active class from all language items
        document.querySelectorAll('.language-list .genre-item').forEach(i => i.classList.remove('active'));
...
function filterEvents() {
    ...
    const activeGenre = document.querySelector('.genre-list:not(.language-list) .genre-item.active').dataset.genre;
    const activeLanguage = document.querySelector('.language-list .genre-item.active').dataset.genre;
    ...
    return events.filter(event => {
        ...
        const languageMatch = activeLanguage === 'all' || event.language === activeLanguage;

        return dateMatch && locationMatch && searchMatch && genreMatch && languageMatch;
    });
}
...
function createEventCard(event) {
    ...
    card.innerHTML = `
        ...
        <div class="event-details">
            ...
            <p class="event-info"><i class="fas fa-language"></i> ${event.language.charAt(0).toUpperCase() + event.language.slice(1)}</p>
...
// Add event listeners for language filter
document.querySelectorAll('.language-item').forEach(item => {
    item.addEventListener('click', () => {
        document.querySelector('.language-item.active')?.classList.remove('active');
        item.classList.add('active');
        renderEvents();
    });
});
